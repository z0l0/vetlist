#!/usr/bin/env python3
"""
VetScore Calculator for USA Veterinarians - VetList.org

Calculates VetScores specifically for the USA professionals file.
"""
...
# Import the scoring logic from the main script
import sys
sys.path.append(os.path.dirname(__file__))

# ============================================================================
# SCORING CONFIGURATION (Same as main script)
# ============================================================================
...
# ============================================================================
# MAIN SCORING FUNCTION (Same as main script)
# ============================================================================
...
def calculate_vetscore(profile):
    ...
    # --- SERVICES & ANIMALS ---
    # Check both specialization and animals_treated for services
    ...
    # Check both pet_types_served (enriched) and animals_treated (original)
    pet_types = safe_parse_json(profile.get('pet_types_served'), [])
    if not pet_types:
        pet_types = safe_parse_json(profile.get('animals_treated'), [])
...
# ============================================================================
# MAIN EXECUTION FOR USA DATA
# ============================================================================

def main():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_file = os.path.join(script_dir, '../data/professionals-usa.csv')
    output_file = os.path.join(script_dir, '../data/professionals-usa.csv')
    report_file = os.path.join(script_dir, '../data/vetscore-usa-report.json')
    
    print('🇺🇸 VetScore Calculator - USA Veterinarians')
    print('==========================================\n')
    ...
    print(f'📊 Processing {len(profiles)} USA clinics...\n')
    ...
    print(f'Quality (60+): {len([s for s in scores if s >= 60])}')
    print(f'Elite (80+):   {len([s for s in scores if s >= 80])}')
    ...
    print(f'\n✅ Updated {output_file} with VetScore data')
    ...
    report = {
        ...
        'statistics': {
            ...
            'quality_60_plus': len([s for s in scores if s >= 60]),
            'elite_80_plus': len([s for s in scores if s >= 80]),
        },
    ...
    print('\n🎯 VetScore calculation complete for USA veterinarians!')
